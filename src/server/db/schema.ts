...
export const campaigns = pgTable(
  ...
  (table) => [
    index("campaign_status_dates_idx").on(
      table.status,
      table.startsAt,
      table.endsAt,
    ),
...
export const submissions = pgTable(
  ...
  (table) => [
    ...
    index("submission_creator_created_at_idx").on(
      table.creatorId,
      table.createdAt,
    ),
    check(
      "submission_post_url_not_blank",
      sql`length(btrim(${table.postUrl})) > 0`,
    ),
    check(
...
export const submissionMetrics = pgTable(
  ...
  (table) => [
    uniqueIndex("submission_metric_submission_date_unique").on(
      table.submissionId,
      table.capturedAt,
    ),
    check(
      "submission_metric_counts_nonnegative",
      sql`${table.views} >= 0 and ${table.likes} >= 0 and ${table.comments} >= 0`,
    ),
  ],
);
